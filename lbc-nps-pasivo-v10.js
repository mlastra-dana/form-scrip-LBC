(function () {
  ...
  function isPage1() {
    return getPageNumber() === "1" || isVisible(get(CONFIG.questionOneLiId)) || isVisible(get(CONFIG.questionTwoLiId));
  }
  ...
  function hideOriginalPage2Text(form) {
    Array.from(form.querySelectorAll("li, div, section")).forEach(function (el) {
      if (el.id === "lbc_pasivo_context_li") return;
      if (el.id === "lbc_pasivo_context_card") return;
  ...
  function styleRecommendationCheckboxGroup(li) {
    ...
    if (label) {
      var text = normalizeLower(label.innerText || label.textContent || "");

      if (text.indexOf("encontramos 2 opciones") !== -1) {
  ...
  function bindProductInterestClick() {
    var form = get(CONFIG.formId);
    var next = get(CONFIG.primaryButtonId);
  ...
  function styleBackLinks() {
    ...
    Array.from(scope.querySelectorAll("a, button, input[type='button'], input[type='submit']")).forEach(function (el) {
      ...
      if (el.id === CONFIG.finalSubmitId) return;
  ...
  function styleAllFields() {
    ...
    if (p3) {
      ...
      Array.from(get(CONFIG.formId).querySelectorAll("li")).forEach(function (li) {
        var text = normalizeLower(li.innerText || li.textContent || "");

        if (text.indexOf("teléfono") !== -1 || text.indexOf("telefono") !== -1 || text.indexOf("email") !== -1 || text.indexOf("correo") !== -1) {
          styleTextField(li);
        }
      });
    ...
    if (p2) {
      hideOriginalPage2Text(get(CONFIG.formId));
      createPage2ContextCard(get(CONFIG.formId));
    }
  ...
  function applyMobile() {
    ...
    setStyle(get("lbc_nps_header"), {
      ...
      "padding": p3 ? "20px 18px 18px 18px" : "20px 18px 22px 18px"
    });
