(function () {
  ...
  function observeConditionalFields() {
    var target = get(CONFIG.formId);
    if (!target || target.getAttribute("data-lbc-observer")) return;

    target.setAttribute("data-lbc-observer", "true");
  ...
  function applyFormStyle() {
    ...
    styleAllFields();
    styleSubmit();
    observeConditionalFields();
    applyMobile();

    console.log("LBC NPS promotor v1 aplicado correctamente.");
    return true;
  }
