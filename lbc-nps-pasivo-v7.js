(function () {
  ...
  function injectWhiteSelectionStyles() {
    if (document.getElementById("lbc_nps_pasivo_white_selection_v7")) return;

    var style = document.createElement("style");
    style.id = "lbc_nps_pasivo_white_selection_v7";
  ...
  function forceWhiteFieldBackgrounds(form) {
  ...
  function isRecommendationField(li) {
    ...
    var text = normalizeLower(li.textContent || "");
  ...
  function isContactField(li) {
    var label = getFieldLabel(li);
    var text = normalizeLower(li.textContent || "");

    return (
      label.indexOf("canal preferido") !== -1 ||
      text.indexOf("whatsapp") !== -1 && text.indexOf("llamada") !== -1 && text.indexOf("email") !== -1
    );
  }
  ...
  function styleRecommendationCheckboxGroup(li) {
    ...
    Array.from(li.querySelectorAll("span")).forEach(function (span) {
      ...
      var optionText = normalizeLower(getOptionText(span));
      var data = RECOMMENDATIONS[optionText] || {
        icon: "✓",
    ...
    Array.from(li.querySelectorAll("input[type='checkbox']")).forEach(function (checkbox) {
      ...
      if (!checkbox.getAttribute("data-lbc-nps-bound")) {
        ...
        checkbox.addEventListener("change", function () {
          updateOptionState();
          forceWhiteFieldBackgrounds(get(CONFIG.formId));
  ...
  function styleAllFields(form) {
    Array.from(form.querySelectorAll("li")).forEach(function (li) {
      if (li.id === CONFIG.buttonsLiId) return;
    ...
    updateOptionState();
    forceWhiteFieldBackgrounds(form);
  }
  ...
  function applyFormStyle() {
    ...
    console.log("LBC NPS pasivo v7 aplicado correctamente.");
    return true;
  }
