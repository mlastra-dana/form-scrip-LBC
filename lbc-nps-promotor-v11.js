(function () {
  ...
  function injectStyles() {
    if (document.getElementById("lbc_nps_promotor_v11_styles")) return;

    var style = document.createElement("style");
    style.id = "lbc_nps_promotor_v11_styles";
  ...
  function createProductContextCard(form) {
    ...
    setStyle(get("lbc_product_context_card"), {
      ...
      "background": "linear-gradient(135deg, #111c44 0%, #1d3f82 56%, #43238f 100%)",
      "background-color": "#173b6c",
      "background-image": "linear-gradient(135deg, #111c44 0%, #1d3f82 56%, #43238f 100%)",
      "border": "0",
      "border-radius": "22px",
      "box-shadow": "0 18px 34px rgba(17, 28, 68, 0.20)",
  ...
  function styleHeader() {
    ...
    setStyle(get("lbc_nps_thanks_icon"), {
      ...
      "height": "56px",
      "border-radius": "18px",
      "background": "linear-gradient(135deg, #3564ff 0%, #6d28d9 100%)",
      "color": "#ffffff",
      "font-size": "30px",
      "font-weight": "800",
      "line-height": "1",
      "margin": "0 0 18px 0",
      "box-shadow": "0 14px 28px rgba(53, 100, 255, 0.22)"
    });
  ...
  function styleRecommendationCheckboxGroup(li) {
    ...
    Array.from(li.querySelectorAll("span")).forEach(function (span) {
      ...
      setStyle(span, {
        ...
        "background-color": "#ffffff",
        "background-image": "none",
        "border": data.recommended ? "1px solid #8aa8ff" : "1px solid #dbe7f6",
        "border-radius": "18px",
        "box-shadow": "none",
        "color": "#475467",
        "font-size": "12px",
        ...
        if (checkbox.checked) {
          ...
          if (li.id === CONFIG.recommendationLiId) {
            ...
            setStyle(span, {
              ...
              "background-color": "#ffffff",
              "background-image": "none",
              "border-color": data && data.recommended ? "#8aa8ff" : "#dbe7f6",
              "color": "#475467"
            });
          } else {
            setStyle(span, {
  ...
  function styleBackLinks() {
    Array.from(document.querySelectorAll("#" + CONFIG.formId + " a, #" + CONFIG.formId + " button, #" + CONFIG.formId + " input[type='button']")).forEach(function (el) {
      var text = normalizeLower(el.value || el.innerText || el.textContent || "");

      if (text.indexOf("volver") === -1 && text.indexOf("anterior") === -1) return;
  ...
  function styleSubmit() {
    ...
    if (next && isVisible(next)) {
      next.value = CONFIG.nextText;

      var slot = get("lbc_product_context_cta_slot");
      if (slot && next.parentNode !== slot) {
  ...
  function applyFormStyle() {
    ...
    console.log("LBC NPS promotor v11 aplicado correctamente.");
    return true;
  }
