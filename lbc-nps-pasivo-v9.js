(function () {
  ...
  var CONFIG = {
    ...
    introCardText: "Selecciona el aspecto que más influyó en tu experiencia.",
  ...
  function getCurrentPage() {
    var pageInput = document.querySelector("#" + CONFIG.formId + " input[name='page_number']");
    return pageInput ? String(pageInput.value || "1") : "1";
  }
  ...
  function injectStyles() {
    if (document.getElementById("lbc_nps_pasivo_v9_styles")) return;

    var style = document.createElement("style");
    style.id = "lbc_nps_pasivo_v9_styles";
    ...
    style.textContent =
      ...
      "#form_8568 textarea," +
      "#form_8568 textarea:hover," +
      "#form_8568 textarea:focus," +
      "#form_8568 select," +
  ...
  function styleHeader() {
    ...
    setStyle(get("lbc_nps_header"), {
      ...
      "padding": firstPage ? "24px 22px 20px 22px" : "20px 22px 10px 22px",
      "background": "linear-gradient(180deg, #ffffff 0%, #f7faff 100%)",
      "border-bottom": firstPage ? "1px solid #dbe7f6" : "0"
    });
    ...
    setStyle(get("lbc_nps_logo_area"), {
      ...
      "width": "100%",
      "height": firstPage ? "64px" : "48px",
      "margin": firstPage ? "0 0 22px 0" : "0",
      "padding": "0",
      "text-align": "center"
    });
    ...
    setStyle(get("lbc_nps_logo"), {
      "display": "block",
      "width": firstPage ? "230px" : "170px",
      "max-width": firstPage ? "230px" : "170px",
      "height": "auto",
      "max-height": firstPage ? "64px" : "48px",
      "object-fit": "contain",
      "margin": "0 auto"
    });
  ...
  function getLabel(li) {
    ...
    return normalizeLower(label ? label.textContent : "");
  }
  ...
  function isRecommendationField(li) {
    var text = normalizeLower(li ? li.textContent : "");
    return text.indexOf("revisar mi cobertura actual") !== -1 && text.indexOf("accidentes personales") !== -1;
  }

  function isPhoneField(li) {
    var label = getLabel(li);
    return label.indexOf("teléfono") !== -1 || label.indexOf("telefono") !== -1;
  }

  function isEmailField(li) {
    var label = getLabel(li);
    return label === "email" || label.indexOf("correo") !== -1;
  }
  ...
  function configureBoliviaPhone(form) {
    Array.from(form.querySelectorAll("li")).forEach(function (li) {
      if (!isPhoneField(li)) return;

      var phone = li.querySelector("input[type='text'], input[type='tel']");
      if (!phone) return;
      ...
      var hintId = "lbc_phone_hint_" + li.id;
  ...
  function configureEmailField(form) {
    Array.from(form.querySelectorAll("li")).forEach(function (li) {
      if (!isEmailField(li)) return;

      var email = li.querySelector("input[type='text'], input[type='email']");
      if (!email) return;
  ...
  function styleSubmitAndPrevious() {
    var li = get("li_buttons");
    var submit = get("submit_primary") || get("submit_form");
    ...
    if (submit) {
      if (normalizeLower(submit.value).indexOf("continuar") !== -1) {
        submit.value = CONFIG.continueText;
      } else {
    ...
    Array.from(document.querySelectorAll("#" + CONFIG.formId + " a, #" + CONFIG.formId + " button, #" + CONFIG.formId + " input[type='button'], #" + CONFIG.formId + " input[type='submit']")).forEach(function (el) {
      var text = normalizeLower(el.value || el.innerText || el.textContent || "");

      if (text.indexOf("anterior") === -1 && text.indexOf("volver") === -1) return;
      if (el === submit) return;

      if (el.tagName === "A") {
        el.innerHTML = "← Anterior";
      } else if ("value" in el) {
        el.value = "← Anterior";
      } else {
  ...
  function styleAllFields(form) {
    Array.from(form.querySelectorAll("li")).forEach(function (li) {
      if (li.id === "li_buttons") return;

      if (isRecommendationField(li)) {
        styleRecommendationCheckboxGroup(li);
        return;
      }

      if (li.querySelector("input[type='checkbox']")) {
        styleStandardCheckboxGroup(li);
        return;
      }

      if (li.querySelector("input[type='text'], input[type='email'], input[type='tel'], textarea, select")) {
        styleTextField(li);
      }
    });

    configureBoliviaPhone(form);
    configureEmailField(form);
    updateOptionState();
  ...
  function applyMobile() {
    ...
    setStyle(get("lbc_nps_header"), {
      ...
      "padding": firstPage ? "20px 18px 18px 18px" : "18px 18px 8px 18px"
    });

    setStyle(get("lbc_nps_logo_area"), {
      "height": firstPage ? "56px" : "44px",
      "margin": firstPage ? "0 0 18px 0" : "0"
    });

    setStyle(get("lbc_nps_logo"), {
      "width": firstPage ? "190px" : "150px",
      "max-width": firstPage ? "190px" : "150px",
      "max-height": firstPage ? "56px" : "44px"
    });
  ...
  function applyFormStyle() {
    ...
    styleHeader();
    styleAllFields(form);
    styleSubmitAndPrevious();
    applyMobile();

    console.log("LBC NPS pasivo v9 aplicado correctamente.");
    return true;
  }
